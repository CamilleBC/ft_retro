...
/*   Updated: 2019/05/26 22:15:34 by cbaillat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void MainScreen::print_object(BluePrint const &object, Point coord) {
    for (size_t i = 0; i < object.size; ++i) {
        wattron(get_win(), COLOR_PAIR(object.colour_pair));
        wattron(get_win(), COLOR_PAIR(2));
        mvwprintw(get_win(), coord.y + i + get_box(),
                  coord.x * x_factor + get_box(), object.texture[i].c_str());
        wattroff(get_win(), COLOR_PAIR(object.colour_pair));
    }
}
